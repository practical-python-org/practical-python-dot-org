...
It fetches the right Python for you, so that's the only thing to install.
...
That serves the site at `http://127.0.0.1:8000` and rebuilds when you save a
file.
...
Broken internal links and dead anchors are build failures, not warnings. CI runs
the same command, so if it passes here it passes there.

Install the git hooks once and you'll get that check automatically on every
commit:
...
Step 2 isn't optional. Zensical can derive navigation from the folder structure,
but that sorts alphabetically and would put Projects ahead of Rules, so the nav
is written out.

[CONTRIBUTING.md](CONTRIBUTING.md) covers the writing conventions, which matter
